import type { TrailerStream } from '@/lib/trailer-stream';
...
const minifyGraphql = (query: string): string => query.replace(/ {4}/g, '');

const VIDEO_GALLERY_FRAGMENT = minifyGraphql(`
...
const TITLE_VIDEO_GALLERY_QUERY = minifyGraphql(`
...
const TITLE_VIDEO_GALLERY_PAGINATION_QUERY = minifyGraphql(`
...
const VIDEO_PLAYBACK_QUERY = minifyGraphql(`
...
async function listImdbTrailers(imdbId: string): Promise<ImdbVideoNode[]> {
  const variables = {
    const: imdbId,
    first: 50,
    filter: {
  ...
  if (strip?.edges) {
    for (const edge of strip.edges) {
      if (edge.node) videos.push(edge.node);
    }
  }
...
const scoreTrailer = (video: ImdbVideoNode): number => {
...
export const selectBestImdbTrailerId = (
  ...
  let best = trailers[0];
  let bestScore = scoreTrailer(best);
  for (let i = 1; i < trailers.length; i += 1) {
    const candidate = trailers[i];
    const score = scoreTrailer(candidate);
    if (score > bestScore) {
      best = candidate;
      bestScore = score;
    }
...
async function fetchImdbPlaybackStreams(
  videoId: string
): Promise<TrailerStream[]> {
...
export async function fetchImdbTrailerStreams(
  imdbId: string
): Promise<TrailerStream[]> {
